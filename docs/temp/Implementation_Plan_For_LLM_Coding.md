...
| 10b.13 | `server/src/types.ts` | Extend the `AuditAction` union (currently ends at `'RETENTION_TICK_RUN'`, no experiment/recommendation values exist yet) with `'AI_EXPERIMENT_CREATED' \| 'AI_EXPERIMENT_STATUS_CHANGED' \| 'AI_EXPERIMENT_PROMOTED' \| 'AI_RECOMMENDATION_APPLIED' \| 'AI_RECOMMENDATION_DISMISSED'`. This is a real, required code change the rest of 10b/10c depend on — call it out explicitly rather than leaving it implied by "writes to audit_log." |
...
| 10b.21 | `server/__tests__/ai/experimentCertification.test.ts` | A provider with an env var present but no passing Phase 1.6 contract-suite run cannot be added as an experiment variant; a certified-and-configured provider can (10b.10). |
| 10b.22 | `server/__tests__/ai/groundTruth.test.ts` | Priority ordering of the three ground-truth signals (10b.12): an admin review-queue decision wins over a conflicting user edit; a user edit wins over a conflicting fixture label when no review decision exists; the correct signal source is tagged on the resulting `ai_ab_test_metrics` row. |
...
- **`server/__tests__/ai/experimentCertification.test.ts`**: A provider with an env var but no passing provider-contract certification cannot be added to an experiment variant; a certified-and-configured one can.
- **`server/__tests__/ai/groundTruth.test.ts`** (new, closes a real gap): Ground-truth signal priority ordering (10b.12) — an admin review-queue decision outranks a conflicting user edit; a user edit outranks a conflicting fixture label when no review decision exists; the resulting `ai_ab_test_metrics` row is tagged with the correct signal source.
...
- **`server/__tests__/deploy/authorization.test.ts`**: Mutating production scripts require `github.actor` and allow only Bryan/Tristan; local `--dry-run` remains side-effect-free.
