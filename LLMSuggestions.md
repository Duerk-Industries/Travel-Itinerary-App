...
Status: `Substantial` (shared client data layer in `app/utils/apiClient.ts` — `requestJson` + `ApiClientError` — used by `useTripsData`, `useGroupInvites`, `useFollowedTrips`, and `useSelectedFollowedTripDetails`. 13 hooks now extracted: `useTripsData`, `useAsyncItineraryPolling`, `usePolling`, `useGroupInvites`, `useFollowedTrips`, `useAuthFlowState`, `useAccountProfile`, `useSelectedFollowedTripDetails`, `useCreateTripWizard`, `useChatState`, `useAuthSession` (core identity cluster: userToken/userName/userEmail/userId/userRole + `applySession`/`clearSessionState` used at login/session-restore/logout), `usePersistedState`, `useConnectionState`. Types split into `app/types/invites.ts`. `App.tsx` down to ~5307 lines from 5507 baseline (-200). Remaining inline logic: password-setup POST + email-resend handlers, 30+ modal/form blocks, per-trip data fetchers (traits/fellows/family).)
