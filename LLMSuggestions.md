...
Status: `Substantial` (shared client data layer in `app/utils/apiClient.ts` — `requestJson` + `ApiClientError` — used by `useTripsData`, `useGroupInvites`, `useFollowedTrips`, and `useSelectedFollowedTripDetails`. 12 hooks now extracted: `useTripsData`, `useAsyncItineraryPolling`, `usePolling`, `useGroupInvites`, `useFollowedTrips`, `useAuthFlowState` (password-setup/first-login/auth-error flow state), `useAccountProfile` (profile + map/appearance preferences with persistence), `useSelectedFollowedTripDetails` (fetches the full Trip for a followed selection), `useCreateTripWizard` (name + group picker + default-group effect + submit validation), `useChatState` (presence + unread + Socket.IO join-trip subscription), `usePersistedState`, `useConnectionState`. Types split into `app/types/invites.ts`. `App.tsx` shrank 5507 → 5297 lines (-210 cumulative). Core auth-session tokens (userToken/userEmail/userId/userRole) still live inline because they are referenced by hundreds of call sites; the login/password-setup logic also remains inline.)
