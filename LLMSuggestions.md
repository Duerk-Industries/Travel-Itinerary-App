...
Status: `Substantial` (shared client data layer in `app/utils/apiClient.ts` — `requestJson` + `ApiClientError` — now used by `useTripsData`, `useGroupInvites`, `useFollowedTrips`, `useSelectedFollowedTripDetails`, `useAuthFlowState`, and `useTraits`. 14 hooks extracted: `useTripsData`, `useAsyncItineraryPolling`, `usePolling`, `useGroupInvites`, `useFollowedTrips`, `useAuthFlowState` (now also owns `completeInitialPasswordSetup` POST + `resendConfirmationEmail` POST with validation), `useAccountProfile`, `useSelectedFollowedTripDetails`, `useCreateTripWizard`, `useChatState`, `useAuthSession` (core identity), `useTraits` (traits list + demographics + seed-selected-on-fetch), `usePersistedState`, `useConnectionState`. Types split into `app/types/invites.ts`. `App.tsx` down to **5255 lines** from 5507 baseline (**-252 cumulative**). Remaining inline: ~30 modal/form blocks (presentational, lower-leverage), per-trip data fetchers for family + fellow-travelers, and the login/register form state (`authForm`/`authMode`).)
