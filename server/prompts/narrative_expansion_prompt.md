...
> **Superseded — documentation only.** This file is not loaded by the application. The live daily
> expansion stage is `prompts/p2_days.md` (with its JSON/template counterpart), executed through the
> p0–p4 pipeline in `itineraryPromptPlanService.ts`. Do not edit this file to change runtime behavior.
