...
describe('db provider selection', () => {
  ...
  it('honors DB_PROVIDER when set', async () => {
    await withEnv({ DB_PROVIDER: 'memory', USE_IN_MEMORY_DB: undefined, FIRESTORE_EMULATOR_HOST: undefined }, async () => {
      const db = await loadDb();
      expect(db.getCurrentDbProvider()).toBe('memory');
    });

    await withEnv({ DB_PROVIDER: 'dynamodb', USE_IN_MEMORY_DB: undefined, FIRESTORE_EMULATOR_HOST: undefined }, async () => {
      const db = await loadDb();
      expect(db.getCurrentDbProvider()).toBe('dynamodb');
    });

    await withEnv({ DB_PROVIDER: 'firebase', USE_IN_MEMORY_DB: undefined, FIRESTORE_EMULATOR_HOST: undefined }, async () => {
      const db = await loadDb();
  ...
  it('throws on unknown provider', async () => {
    await withEnv({ DB_PROVIDER: 'not-a-provider', USE_IN_MEMORY_DB: undefined, FIRESTORE_EMULATOR_HOST: undefined }, async () => {
      const db = await loadDb();
...
describe('db lifecycle', () => {
  it('initializes and closes without error (memory)', async () => {
    await withEnv({ DB_PROVIDER: 'memory', FIRESTORE_EMULATOR_HOST: undefined }, async () => {
      const db = await loadDb();
