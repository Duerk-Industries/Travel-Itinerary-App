...
describe('trip blog storage reconciliation', () => {
  ...
  beforeAll(async () => {
    ...
    const trip = await request(app).post('/api/trips/wizard').set('Authorization', `Bearer ${token}`).send({ name: 'Reconcile Trip', startDate: TRIP_DAY, endDate: TRIP_DAY, participants: [] }).expect(201);
  ...
  it('hides media when account is over capacity during global reconciliation', async () => {
    ...
    const init1 = await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'rec-1').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 1024 * 1024 }).expect(201);
    ...
    const init2 = await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'rec-2').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 1024 * 1024 }).expect(201);
