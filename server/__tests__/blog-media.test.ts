...
describe('trip blog media quota lifecycle', () => {
  ...
  beforeAll(async () => {
    ...
    const trip = await request(app).post('/api/trips/wizard').set('Authorization', `Bearer ${token}`).send({ name: 'Media Trip', startDate: TRIP_DAY, endDate: TRIP_DAY, participants: [] }).expect(201);
  ...
  it('reserves, completes, and accounts an uploader-owned photo', async () => {
    const init = await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'photo-1').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 2048 }).expect(201);
  ...
  it('persists a client-supplied capturedAt and nulls an unparseable one', async () => {
    const good = await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'photo-capturedat').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 512, capturedAt: `${TRIP_DAY}T14:30:00` }).expect(201);
    ...
    const bad = await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'photo-capturedat-bad').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 512, capturedAt: 'not-a-date' }).expect(201);
    ...
    expect(goodAsset.capturedAt).toContain(TRIP_DAY);
    expect(badAsset.capturedAt).toBeNull();
  });

  it('rejects unsupported image formats before reserving bytes', async () => {
    await request(app).post(`/api/trips/${tripId}/blog/media/upload-init`).set('Authorization', `Bearer ${token}`).set('Idempotency-Key', 'photo-heic').send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/heic', byteSize: 2048 }).expect(400);
  });
  ...
  it('atomically charges generated rendition bytes above the original reservation', async () => {
    ...
    const init = await request(app)
      ...
      .send({ dayDate: TRIP_DAY, mediaKind: 'photo', mimeType: 'image/jpeg', byteSize: 1000 })
      .expect(201);
  ...
  it('exposes the blog_items id (not the media asset id) via GET /blog, and Remove actually removes it', async () => {
    ...
    const day = blog.body.days.find((candidate: any) => candidate.localDate === TRIP_DAY);
    ...
    const dayAfter = afterDelete.body.days.find((candidate: any) => candidate.localDate === TRIP_DAY);
