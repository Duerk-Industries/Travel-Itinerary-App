...
import { refreshMeanVector } from '../src/services/meanVectorService';
...
describe('Itinerary Cache v2 Logic', () => {
  ...
  const mockBlocks: ActivityBlock[] = [
    {
      ...
      interest_weights: {
        outdoors: 1, adventure: 1, culture: 10, food: 2, nightlife: 1,
        relaxing: 3, photography: 5, authentic_local: 4, iconic_landmarks: 8,
      },
  ...
  it('calculates mean-centered cosine similarity correctly', () => {
    ...
    const unlikeWeights = {
      outdoors: 10, adventure: 10, culture: 1, food: 1, nightlife: 1,
      relaxing: 1, photography: 1, authentic_local: 1, iconic_landmarks: 1,
    };
  ...
  it('selects blocks deterministically based on fit', async () => {
    // Mock getMeanVector to return mockMean
    const userWeights = {
      outdoors: 1, adventure: 1, culture: 10, food: 2, nightlife: 1,
      relaxing: 3, photography: 5, authentic_local: 4, iconic_landmarks: 8,
    };
