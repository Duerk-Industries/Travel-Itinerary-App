export type TravelEndpointFact = {
  date: string;
  localTime?: string | null;
  isLongHaul?: boolean;
  durationHours?: number | null;
  /** A terminal leg with no booked local time is treated conservatively. */
...
export const buildArrivalDepartureFacts = (params: { arrival?: TravelEndpointFact | null; departure?: TravelEndpointFact | null; departureBufferMinutes?: number }): LogisticsFact[] => {
  ...
  if (params.arrival) {
    ...
    const terminalOnly = Boolean(params.arrival.terminalOnly);
    facts.push({
      date: params.arrival.date, kind: 'arrival', maxActivities: terminalOnly ? 0 : heavy ? 1 : 2,
      ...(arrivalMinutes == null ? {} : { earliestActivityTime: minutesToClock(Math.min(23 * 60 + 59, arrivalMinutes + 150)) }),
      note: terminalOnly
        ? 'Travel day: no activities scheduled; allow terminal, transfer, check-in, and recovery time.'
        : heavy ? 'Heavy arrival day: allow immigration, baggage, transfer, check-in, and recovery time.' : 'Arrival day: keep plans flexible after check-in.',
    });
  ...
  if (params.departure) {
    ...
    const terminalOnly = Boolean(params.departure.terminalOnly);
    facts.push({
      date: params.departure.date, kind: 'departure', maxActivities: terminalOnly || (departureMinutes != null && departureMinutes < 12 * 60) ? 0 : 1,
      ...(departureMinutes == null ? {} : { latestActivityTime: minutesToClock(Math.max(0, departureMinutes - buffer)) }),
      note: terminalOnly
        ? `Departure day: protect at least ${buffer} minutes for checkout, terminal travel, and processing. Travel day: no activities scheduled.`
        : `Departure day: protect at least ${buffer} minutes for checkout, terminal travel, and processing.`,
    });
  }
