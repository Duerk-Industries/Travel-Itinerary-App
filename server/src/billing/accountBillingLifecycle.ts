 ...
 *
 * Throws if any subscription cannot be cancelled. Deleting the local account
 * while Stripe can still charge it would remove the durable customer mapping
 * needed for support and automated cleanup.
 */
export const cancelAllSubscriptionsForUser = async (userId: string): Promise<void> => {
  ...
  for (const sub of subscriptions) {
    ...
    try {
      ...
      failures.push(sub.stripeSubscriptionId);
    }
  }
  if (failures.length > 0) {
    throw new Error(`Unable to cancel Stripe subscriptions: ${failures.join(', ')}`);
  }
};
