...
if [[ "$DRY_RUN" != "1" ]]; then
  gcloud run deploy "$TEST_SERVICE_NAME" \
    ...
    --update-env-vars "GCLOUD_PROJECT_ID=$GCLOUD_PROJECT_ID,WEB_URL=$TEST_DOMAIN,BACKEND_URL=$TEST_DOMAIN,GOOGLE_CALLBACK_URL=$TEST_DOMAIN/api/auth/google/callback,FIRESTORE_DATABASE_ID=$TEST_FIRESTORE_DATABASE_ID,AI_CAPTURE_BUCKET=$TEST_AI_CAPTURE_BUCKET,DB_PROVIDER=firebase" \
