...
exports.parseFlightText = void 0;
...
const toCityFromCode = (code, fallback) => {
    if (!code)
        return fallback ?? '';
...
const parseFlightSegments = (text) => {
    const segments = [];
    const segmentRegex = /([A-Z]{3})\s*(?:to|->|→|—|–|-)\s*([A-Z]{3})[^\n]*?(\d{1,2}:\d{2}\s*(?:AM|PM)?)?[^\n]*?(\d{1,2}:\d{2}\s*(?:AM|PM)?)/gi;
...
const pickEarliestDate = (candidates) => {
    let earliest;
...
const extractDecimalAmounts = (value) => Array.from(value.matchAll(new RegExp(`(${MONEY_AMOUNT_PATTERN})`, 'gi')))
    .map((match) => match[1]?.replace(/,/g, ''))
    .filter((amount) => Boolean(amount));
const extractCurrencyTaggedAmounts = (value) => Array.from(value.matchAll(new RegExp(`(?:[$€£]|\\b(?:${MONEY_CODE_PATTERN})\\b)\\s*(${MONEY_AMOUNT_PATTERN})|(${MONEY_AMOUNT_PATTERN})\\s*(?:\\b(?:${MONEY_CODE_PATTERN})\\b)`, 'gi')))
    .map((match) => (match[1] ?? match[2] ?? '').replace(/,/g, ''))
    .filter((amount) => Boolean(amount));
const extractCostFromText = (text) => {
    const normalized = text.replace(/\u00a0/g, ' ').replace(/[ \t]+/g, ' ');
    const labeledWindowRegex = /\b(?:receipt|total(?:\s+price)?(?:\s+of\s+your\s+trip)?|amount(?:\s+paid)?|payment(?:\s+amount)?|trip purchased)\b[\s\S]{0,160}/gi;
    for (const match of normalized.matchAll(labeledWindowRegex)) {
        ...
        if (currencyTagged.length)
            return currencyTagged[currencyTagged.length - 1];
    ...
    if (globalCurrencyTagged.length)
        return globalCurrencyTagged[0];
...
const parseFlightText = (text) => {
    ...
    if (bookingMatch)
        parsed.bookingReference = bookingMatch[1].toUpperCase();
    if (!parsed.bookingReference) {
        const airlineConfMatch = text.match(/airline\s+confirmation[:\s]+([A-Z0-9]{4,8})/i);
        if (airlineConfMatch)
            parsed.bookingReference = airlineConfMatch[1].toUpperCase();
    }
    ...
    if (flightMatch)
        parsed.flightNumber = flightMatch[1].replace(/\s+/g, '').toUpperCase();
    ...
    const namedDateRegex = /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2},?\s+\d{4}\b/gi;
    const namedDates = Array.from(text.matchAll(namedDateRegex)).map((m) => m[0]);
    const bestDate = pickEarliestDate([...isoDates, ...namedDates]);
    if (bestDate)
        parsed.departureDate = bestDate;
    const layoverDetail = text.match(/\(([A-Z]{3})\s*[—-]\s*(\d{1,2})h\s*(\d{1,2})m\)/i);
    const layoverMatch = layoverDetail ||
    ...
    if (!parsed.departureTime && timeMatches[0])
        parsed.departureTime = timeMatches[0].toUpperCase();
    if (!parsed.arrivalTime && timeMatches[1])
        parsed.arrivalTime = timeMatches[1].toUpperCase();
    ...
    if (travelersForBulk.length && segments.length && parsed.departureDate) {
        ...
        const perFlightCost = totalCost && travelersForBulk.length && segments.length
        ...
        travelersForBulk.forEach((traveler, travelerIdx) => {
            segments.forEach((segment, segIdx) => {
                ...
                bulk.push({
                    ...
                    layoverLocation: parsed.layoverLocation ??
        ...
        travelersForBulk.forEach((traveler, travelerIdx) => {
            segments.forEach((segment, segIdx) => {
                ...
                duplicated.push({
                    ...base,
                    passengerName: traveler,
                    cost: parsed.cost && segments.length && travelersForBulk.length
...
exports.parseFlightText = parseFlightText;
