...
import { parseFlightText } from '../utils/parsers/flightParser';
...
const main = async () => {
  ...
  if (!target) {
    console.error('Usage: npm run parse:debug -- <file.pdf|image>');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('Primary parsed flight:');
    console.log(JSON.stringify(primary, null, 2));
    console.log('Bulk parsed flights:');
